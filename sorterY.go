...
// when left = 0
func selectY(array ySorter, k, left, right int) {
	length := array.Len()
	for right > left {
		if right-left > 600 {
			...
			var newLeft = ySorterMax(left, int(math.Floor(kf-m*s/n+sd)))
			var newRight = ySorterMin(right, int(math.Floor(kf+(n-m)*s/n+sd)))
			selectY(array, k, newLeft, newRight)
		}
...
type ySorterStack []int

func (s ySorterStack) push(v int) ySorterStack {
	return append(s, v)
}
func (s ySorterStack) pop() (ySorterStack, int) {
	l := len(s)
	return s[:l-1], s[l-1]
}
